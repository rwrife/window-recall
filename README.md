...
> Status: deterministic matching, display-topology mapping, safe immutable previews, selective coordination, and one-step best-effort undo are implemented in Core, plus deterministic end-to-end restore scenarios across laptop-only, external-only, extended, display-removal, negative-coordinate, scale-mismatch, rotated-target, minimized/maximized, ambiguous multi-window, and rejected-move/partial-failure paths. A headless CLI (capture, profiles, plan, apply, undo, export, import, doctor) with documented exit codes, stable `--json` output, conflict-safe import/export, persisted undo receipts, and privacy controls is implemented and tested against an injected fake desktop on Linux, Windows, and macOS runners. The Avalonia UI implements the keyboard-complete capture-review, profile-editor, restore-preview, ambiguity-resolution, confirmed apply/cancel, partial-result, and undo workflow with automation-name, live-region, and non-color-status semantics (verified headless; Narrator/VoiceOver passes remain to be recorded). Windows and macOS adapter vertical slices have deterministic fake-native coverage and opt-in live fixture harnesses; Windows consumes topology-planned multi-monitor bounds and state changes, while macOS remains read-only without Accessibility permission and re-observes authorized AX mutations. Release packaging is automated (portable CLI/app ZIPs for win-x64/osx-x64/osx-arm64, unsigned macOS `.app` wrappers, SHA256SUMS, and a machine-readable artifact manifest that always discloses signing status), and the release checklist in `docs/release-checklist.md` enumerates exactly which integration, install, permission, and screen-reader checks remain manual or unavailable. Live Windows and macOS evidence has not yet been recorded.
...
Platform adapter tests use injected fake native APIs and are not proof that real windows were observed or moved. See [Windows adapter verification](docs/windows-adapter-verification.md) and [macOS adapter verification](docs/macos-adapter-verification.md) for filtering rules, permission behavior, live harness instructions, and clearly separated mock/build/live evidence. UI tests run headless against fake desktops and are not proof a real window moved; see [Avalonia UI verification](docs/avalonia-ui-verification.md) for the automated UI evidence boundary and the manual Narrator/VoiceOver checklist.
