...
Application identity is the running application's bundle identifier plus its executable file URL, converted to an absolute path at the Core boundary. Native window IDs and `AXUIElement` references remain internal to `WindowRecall.Platform.MacOS`; Core sees only randomly generated opaque session IDs. An ID is reused across complete captures only while native window ID, owner PID, bundle identifier, and executable URL all still agree, allowing the restore coordinator to capture its immediate undo snapshot without invalidating an approved plan. Duplicate native observations are ignored. Apply accepts only IDs from the last complete capture and revalidates the same identity before mutation. A cancelled capture does not replace the prior complete session.
